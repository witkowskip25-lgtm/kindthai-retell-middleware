...
function effectiveTherapistMap(raw) {
  const map = { ...raw };
  if (map["Sara"] && !map["Nina"]) { map["Nina"] = map["Sara"]; delete map["Sara"]; }
  const roseCal = process.env.ROSE_CAL_ID;
  if (roseCal) map["Rose"] = roseCal;
  return map;
}

/** Count busy therapists in window (capacity gate) */
...
/** Nearby suggestions within roster & capacity */
async function suggestTwo({ allowedNames, map, startIso, endIso }) {
  ...
  for (const d of deltas) {
    ...
    // pick any free therapist for this shift
    let ok = false;
...
/* ===========================
   POST /booking/create  (enforce roster + 2 rooms)
   =========================== */
router.post("/create", async (req, res) => {
  try {
    const { startIso, endIso, clientName, clientPhone, serviceName, duration, therapist, requestId } = req.body || {};
    ...
    if (!start.isValid || !end.isValid) return res.status(400).json({ ok:false, error:"Invalid ISO times" });

    const weekday = start.weekday; // 1..7
    ...
    if (loadNow >= MAX_ROOMS) {
      ...
      return res.status(409).json({ ok:false, error:"capacity_reached", policy:{ rosterForDay:allowedNames, maxRooms:MAX_ROOMS, loadNow }, suggestions });
    }
    ...
    if (selectedName) {
      if (!allowedNames.includes(selectedName)) {
        const suggestions = await suggestTwo({ allowedNames, map, startIso, endIso });
        return res.status(409).json({ ok:false, error:"therapist_not_scheduled_today", therapistRequested:selectedName, policy:{ rosterForDay:allowedNames, maxRooms:MAX_ROOMS, loadNow }, suggestions });
      }
      const { anyFree } = await isFree(calId, startIso, endIso);
      if (!anyFree) {
        const suggestions = await suggestTwo({ allowedNames, map, startIso, endIso });
        return res.status(409).json({ ok:false, error:`time_not_available_for_${selectedName}`, policy:{ rosterForDay:allowedNames, maxRooms:MAX_ROOMS, loadNow }, suggestions });
      }
    } else {
      ...
      if (!calId) {
        const suggestions = await suggestTwo({ allowedNames, map, startIso, endIso });
        return res.status(409).json({ ok:false, error:"time_not_available_for_any_rostered_therapist", policy:{ rosterForDay:allowedNames, maxRooms:MAX_ROOMS, loadNow }, suggestions });
      }
    }
    ...
    console.error("booking/create error:", safeJson(err?.response?.data || { message: err?.message, stack: err?.stack }));
...
/* ===========================
   POST /booking/reschedule (preflight + phone recovery)
   =========================== */
async function recoverLatestEventForPhone(map, phone, fromIso, toIso) {
  ...
  for (const [name, calId] of Object.entries(map)) {
    try {
      ...
      if (!list || list.length === 0) list = await searchByPrivateProp(calId, start.toISO(), end.toISO(), "clientPhone", phone, 10);
...
router.post("/reschedule", async (req, res) => {
  try {
    ...
    catch (e) { if ((e?.response?.status || e?.code) === 404) evExists = false; else throw e; }
    ...
    if (!evExists) {
      ...
      if (!recovered) return res.status(404).json({ ok:false, error:"event_not_found", detail:"Original event missing and recovery failed." });
    ...
    if (selectedName && !targetCalId) return res.status(404).json({ ok:false, error:`Unknown therapist '${selectedName}'` });
    ...
    if (targetCalId === srcCal) {
      ...
      return res.json({ ok:true, movedCalendar:false, therapist:selectedName, calendarId:srcCal, eventId:updated.id, htmlLink:updated.htmlLink, startIso:newStartIso, endIso:newEndIso });
    }
    ...
    const created = await createEvent(targetCalId, {
      startIso:newStartIso, endIso:newEndIso, summary, description, requestId,
      privateProps: {
        therapist: selectedName,
    ...
    return res.json({ ok:true, movedCalendar:true, therapist:selectedName, calendarId:targetCalId, eventId:created.event.id, htmlLink:created.event.htmlLink, startIso:newStartIso, endIso:newEndIso });
  } catch (err) {
...
router.post("/cancel", async (req, res) => {
  try {
    ...
    if (!eventId || !calendarId) return res.status(400).json({ ok:false, error:"eventId and calendarId are required" });
    await deleteEvent(calendarId, eventId);
    return res.json({ ok:true, eventId, calendarId });
  } catch (err) {
    console.error("booking/cancel error:", safeJson(err?.response?.data || { message: err?.message, stack: err?.stack }));
    const status = err?.response?.status || 500;
    const msg = err?.response?.data?.error?.message || err?.message || "Server error";
    const code = err?.response?.data?.error?.status;
    return res.status(status).json({ ok:false, error: msg, code });
...
router.post("/find", async (req, res) => {
  try {
    ...
    console.error("booking/find error:", safeJson(err?.response?.data || { message: err?.message, stack: err?.stack }));
    const status = err?.response?.status || 500;
    const msg = err?.response?.data?.error?.message || err?.message || "Server error";
    const code = err?.response?.data?.error?.status;
    return res.status(status).json({ ok:false, error: msg, code });
